...
const Transformer = {
    ...
    getDefinitions(sails) {
        ...
        if(sails.config.swagger.definitions){
            for (var property in sails.config.swagger.definitions) {
                if (sails.config.swagger.definitions.hasOwnProperty(property) && !definitions.hasOwnProperty(property)) {
                    definitions[property] = sails.config.swagger.definitions[property];
                }
            }
    ...
    getPaths(sails) {
        ...
        let paths = _.mapValues(pathGroups, pathGroup => {
            return Transformer.getPathItem(sails, pathGroup)
        }) || [];
        var explicitPaths = Transformer.getDefinitionsFromRouteConfig(sails);
        
        for(var route in explicitPaths){
            console.log("Assessing " + route);
            if(explicitPaths.hasOwnProperty(route)){
                console.log("Legit");
                if(!paths.hasOwnProperty(route)){
                    console.log("Paths doesn't have this route");
                    paths[route] = explicitPaths[route];
                    continue;
                }
                console.log("Paths has this route");
                for(var verb in explicitPaths[route]){
                    console.log(route + " with verb " + verb);
                    paths[route][verb] = explicitPaths[route][verb];
                }
            }
        }
        return paths;
    ...
    getDefinitionsFromRouteConfig(sails) {
        ...
        for (var defRoute in swaggerDefinitions) {
            var sPath = (defRoute || '').replace(/(get|post|put|option|delete)? ?/gi, '');
